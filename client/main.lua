...
Citizen.CreateThread(function()
	...
	while true do
		...
		elseif dist <= 3 and inUse then
			sleep = 0
			DrawText3D(coord.x, coord.y, coord.z, 'Someone has already rented a plane. please wait')
		else
			sleep = 3000
		end
		Citizen.Wait(sleep)
...
AddEventHandler('coke:onUse', function()
	...
	DoScreenFadeIn(2000)
	if Config.useMythic then
		QBCore.Functions.Notify("You feel normal now..", "success")
	end
...
function main()
	...
	if Config.useMythic then
		QBCore.Functions.Notify("Go to the airport.", "success")
	end
	...
	while checkPlane do
		...
		if DoesEntityExist(airplane) then
			if GetVehicleEngineHealth(airplane) < 0 then
				if Config.useMythic then
					QBCore.Functions.Notify("Failed, Plane Destroyed", "error")
				end
				TriggerServerEvent('coke:updateTable', false)
				RemoveBlip(blip)
...
function delivery()
	if Config.useMythic then
		QBCore.Functions.Notify("Get on board and pick up the delivery marked on your GPS", "success")
	end
		...
		while delivering do
			...
			if disttocoord <= 30 then
				...
				if IsControlJustPressed(1, 51) then
					...
					QBCore.Functions.Progressbar("picking_", "pickingup..", lockpickTime, false, true, {
					...
					Citizen.Wait(2000)
					QBCore.Functions.Notify("Picked up delivery return to your selected airpoort.", "success")
					Citizen.Wait(2000)
					final()
				end
			else
...
function final()
	QBCore.Functions.Notify("Deliver the plane to the hanger", "success")
		...
		while hangar do
			...
			if IsPedInAnyPlane(GetPlayerPed(-1)) and disttocoord <= 10 then
				...
				if IsControlJustPressed(1, 51) then
					...
					QBCore.Functions.Progressbar("lockpick_vehicledoor", "Lock..", 1000, false, true, {
						disableMovement = true,
						disableCarMovement = true,
						disableMouse = false,
						disableCombat = true,
			...
			if IsControlJustPressed(1, 51) then		
				...
				QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
					if result then
						...
						QBCore.Functions.Notify("You need a coke brick to do this", "error")
						isProcessing = false
					end
				end, 'coke_brick')
			end
...
function processing()
	...
	QBCore.Functions.Progressbar("coke-", "تحطيم الكوك", 30000, false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
