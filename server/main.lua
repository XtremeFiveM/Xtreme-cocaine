...
QBCore.Functions.CreateCallback('coke:pay', function(source, cb)
	...
	if cashamount >= amount then
		...
		TriggerClientEvent("QBCore:Notify", src, "You do not have enough money to start", "error", 4000)
		cb(false)
	end
end)
...
AddEventHandler("coke:processed", function(x,y,z)
	...
	local pick = Config.randBrick
        if 	TriggerClientEvent("QBCore:Notify", src, "Make a bag of coke!!", "Success", 1) then
...
AddEventHandler("coke:processeda", function(x,y,z)
	...
	if 	TriggerClientEvent("QBCore:Notify", src, "Make a big of coke!!", "Success", 1) then
